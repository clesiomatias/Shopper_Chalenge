...
app.post("/ride/estimate", async (req: Request, res: Response) => {
  ...
  try {
    ...
    const drivers : Driver[]=db.data.drivers
...
app.patch("/ride/confirm", async (req: Request, res: Response) => {
  ...
  try {
   db.data.rides
    ...
    return { error_code: "INVALID_DATA", error_description: error as string };
  }
});

//@ts-ignore
app.get("/ride", async (req: Request, res: Response) => {
...
//@ts-ignore
app.get("/map-image", async (req: Request, res: Response) => { 
...
catch (error: any) { console.error('Erro ao buscar imagem do mapa:', error); 
